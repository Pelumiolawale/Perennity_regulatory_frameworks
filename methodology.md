...
## Methodology corrections — engine v0.6.2 (June 2026)

**Not a methodology version bump.** v0.6.2 is an engine release that (a) corrects documentation drift about already-implemented behaviour and (b) tightens the free/paid disclosure boundary without changing per-criterion scoring logic. Methodology version stays at `v3.5`. Engine output for any v3.5 engagement is bit-identical to v0.6.1 EXCEPT for the items below.

**Aggregate verdict — corrects v3.4 and v3.3 historical statements.** The v3.4 and v3.3 sections of this document (lines 238-241 and 335-339) state that the framework `overall_verdict` is hardcoded `"not_applicable"` pending a post-Phase-1 calibration commit. This was true at the moment those sections were written, but the v0.5.0-alpha.2 (commit 1.2) "E4" change-set wired `aggregateProductLabelVerdict` into the runtime BEFORE the calibration commit landed. Since v0.5.0-alpha.2, every SFDR Art 8 / Art 9 framework_result has carried a real computed `overall_verdict` via severity-rank aggregation: `not_aligned` > `partially_aligned` > `insufficient_evidence` > `not_applicable` > `aligned`. The same applies to the three UK SDR frameworks added in v0.6.0. The v3.4 and v3.3 "deferred" claims are superseded by this paragraph as of v0.6.2. The historical sections are preserved (audit-replay anchor) but read them in light of this correction.

**Weight calibration remains genuinely deferred.** Criterion `weight: null` and `verdict_thresholds: { aligned: null, partially_aligned: null, not_aligned: 0 }` in every framework JSON are NOT corrected. A future calibration commit will introduce per-criterion weights to refine framework-level verdicts beyond the severity-rank rule (e.g. allow a framework with one `insufficient_evidence` criterion in a low-weight position to still emit `aligned`). When that lands, this section will be expanded to describe the resulting verdict-derivation. Until then: severity rank is the operative rule, and severity-rank verdicts ARE the canonical engine output — they are NOT placeholders.

**Free-tier `rationale_text` citation strip.** 8 scoring functions' `rationale_text` strings previously carried verbatim regulatory citations (`(FCA PS23/16 ¶X.Y)`), numeric thresholds (`cool ≤1.2 / warm ≤1.3`), or methodology version stamps (`v3.5`). Per the free/paid line, these belong in the paid-tier Report, not in free-tier Snapshot rationale. v0.6.2 strips the offending substrings and relocates citation strings into a new `CriterionResult.regulatory_citations?: string[]` field that is explicitly blocked from `SnapshotOutput` by the structural gate. Per-criterion verdicts are unchanged; only the prose explaining the verdict is cleaned. Paid-tier surfaces continue to receive the citation trail via the framework JSON `regulatory_anchors[]` array (unchanged) plus the new per-criterion `regulatory_citations` field (new).

**UK SDR c1 / c15 cross-framework dep edge case.** When EU Taxonomy 8.1 is scored in the same run as UK SDR Focus or Impact but Tax returns `overall_verdict: "not_applicable"` (developer makes no Taxonomy claim under SFDR Art 8 light-green positioning), the v0.6.0 UK SDR scoring functions fell through to a misleading `not_aligned` rationale ("claim made and rejected") for c1 and `insufficient_evidence` for c15 ("data missing"). v0.6.2 adds explicit `not_applicable` handling: both criteria now return `not_aligned` with rationale citing the unmet dependency on a credible standard (c1) or DNSH-equivalent evidence (c15). No change to verdicts for non-edge cases.
